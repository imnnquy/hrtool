...
(function ($) {
    ...
    EditableSelect.prototype = {
        ...
        */
        ...
        addOption: function (value, first) {
            var li = $('<li>' + value + '</li>');
            var option = $('<option>' + value + '</option>');
            if (first) {
                this.select.prepend(option);
                this.initListItemEvents(li);
                this.wrapper.find('ul').prepend(li);
            } else {
        ...
        delOption: function (index) {
            if (index) {
                if (index > 0) {
                    ...
                    this.wrapper.find('ul li:eq(' + index + ')').remove();
                } else {
                    this.select.children('option:last').remove();
                    this.wrapper.children('ul').children('li:last').remove();
                }
            } else {
                this.select.html('')
                this.wrapper.find('ul li').each(function () { $(this).remove() });
            }
        },
        delCurrentSelected: function () {
            this.select.find('option:contains(' + this.current_value + ')').remove();
            this.wrapper.find('ul li:contains(' + this.current_value + ')').remove();
            this.current_value = '';
            this.old_value = '';
        },
        /**
        ...
        */
        ...
        function (e) {
            switch (e.keyCode) {
                ...
                // Tab                          
                case 9:
                    context.current_value = context.old_value;
        ...
        selectFirstListItem: function () {
            ...
            this.current_value = first.html();
            this.text.val(this.current_value);
        ...
        undoSelect: function () {
            var oldLiItem = this.wrapper.find('li:contains(' + this.old_value + ')');
            var oldOptionItem = this.select.find('option:contains(' + this.old_value + ')');

            //Update old and current value
            var temp_current_value = this.current_value;
            this.current_value = this.old_value;
            this.old_value = temp_current_value;

            //Update GUI and Html
            this.clearSelectedListItem();
            this.select.find('option:selected').removeAttr('selected');
            this.text.val(this.current_value);
            oldOptionItem.attr('selected', 'selected');
            oldLiItem.addClass('selected');
        },
        ...
        */
        pickListItem: function (list_item) {
            if (list_item.length) {
                ...
                this.select.find('option:contains(' + this.current_value + ')').attr('selected', 'selected');
        ...
        setWidths: function () {
            ...
            if (this.settings.width != null) {
                var pWidth = this.settings.width;
                ...
                this.select.width(pWidth + padding_right + padding_left);
                this.select.hide();

                this.text.width(pWidth);
                this.wrapper.width(pWidth + padding_left + padding_right);
                if (this.bg_iframe) {
                    this.bg_iframe.width(pWidth + padding_left + padding_right + 4);
                };
            } else {
